...
Calling the `/dynamicvsm` path is another valid option:

```bash
curl https://xxxxxxx.execute-api.us-east-1.amazonaws.com/dynamicvsm
```

Finally, calling the path, `/cava` is valid as well:
```bash
curl https://xxxxxxx.execute-api.us-east-1.amazonaws.com/cava
```
